 ...
 * @param {Object} config Containing clientId, clientSecret, apiScope,
 *    accessType, responseType and approvalPrompt. Alternatively,
 *    OAuth2.FINISH for the finish flow.
 * @constructor
 */
var OAuth2 = function(adapterName, config) {
  ...
  OAuth2.loadAdapter(adapterName, function() {
    ...
    if (config == OAuth2.FINISH) {
      ...
      data.accessType = config.access_type;
      data.responseType = config.response_type;
      data.approvalPrompt = config.approval_prompt;
      that.setSource(data);
    }
  });
};
...
OAuth2.prototype.updateLocalStorage = function() {
  ...
  var variables = [
    ...
    'accessType', 'responseType', 'approvalPrompt', 'expiresIn', 'refreshToken'
  ];
 ...
 *
 * @param {string} authorizationCode Retrieved from the first step in the
 *                                   process.
 * @param {function} callback Called back with 3 params:
 *                            access token, refresh token and expiry time.
 */
OAuth2.prototype.getAccessAndRefreshTokens =
  ...
  var items =
      that.adapter.accessTokenParams(authorizationCode, that.getConfig());
  var key = null;
  if (method == 'POST') {
 ...
 * @param {function} callback On success, called with access token and
 *                            expiry time.
 * @param {function(!XMLHttpRequest)=} opt_failCallback If defined, invoked
 *     if token refresh fails.
 */
OAuth2.prototype.refreshAccessToken =
    function(refreshToken, callback, opt_failCallback) {
  var xhr = new XMLHttpRequest();
...
OAuth2.prototype.finishAuth = function() {
  ...
  try {
    authorizationCode =
        that.adapter.parseAuthorizationCode(window.location.href);
...
/**
 * @return {boolean} True iff the current access token has expired.
 */
 ...
 *
 * @return {Object} Contains clientId, clientSecret, apiScope, accessType,
 *    responseType and approvalPrompt.
 */
OAuth2.prototype.getConfig = function() {
  ...
  return {
    ...
    approvalPrompt: data.approvalPrompt
  };
};
 ...
 *
 * @param {string} adapterName The name of the JS file.
 * @param {function} callback Called as soon as the adapter has been loaded.
 */
 ...
 * @return {!Object} The adapter for the current page.
 */
    ...
    if (!data.accessToken) {
      ...
      if (data.refreshToken) {
        that.refreshAccessToken(data.refreshToken, function(at, exp) {
          ...
          data.refreshToken = '';
          // Invalid refresh token... just do the popup thing again.
          that.openAuthorizationCodePopup(callback);
        });
      } else {
 ...
 *
 * @return {boolean} True if an access token exists; otherwise false.
 */
